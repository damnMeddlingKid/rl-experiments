...
tf.app.flags.DEFINE_integer("EPOCHS", 4000000, "Number of training episodes.")
...
def evaluate(session):
    ...
    for i in xrange(1):
        for frame in xrange(1000):
            ...
            state = game.play_frames(action)
            total_rewards += state['reward']
            frames.extend(state['frames'])
        game.reset_game()

    with open('/output/another_run.gif', 'wb') as f:  # change the path if necessary
...
def main(_):
    ...
    saver.restore(session, './checkpoint/model.ckpt-400000')
